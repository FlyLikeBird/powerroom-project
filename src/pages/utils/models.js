    ...
    if ( isWarning ){
        ...
        if ( Object.keys(data).length ) {
            infoData = [
                ...
                {title:'加载率', value: data.run_time ? (data.load_time/data.run_time * 100).toFixed(1) : '-- --', unit:'%'}, { title:'实时电流', value: data.ele || '-- --', unit:'A'},
                {title:'加载压力', value: data.loading ? Math.round(data.loading/10) : '-- --', unit:'Bar'}, { title:'卸载压力', value: data.unloading ? Math.round(data.unloading/10) : '-- --', unit:'Bar'},
                {title:'机组排气压力', value: data.grp_air_out ? Math.round(data.grp_air_out/10) : '-- --', unit:'Bar'}, { title:'主机排气温度 ', value: data.main_tmp_out ? Math.round(data.main_tmp_out/10) : '-- --', unit:'℃'},
