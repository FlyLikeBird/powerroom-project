...
function CustomDatePicker({ dispatch, global, onDispatch, size, optionStyle, noToggle, noDay }){
    const { timeType, startDate, endDate } = global;
    const inputRef = useRef();
    return (
        <div className={style['container']} style={optionStyle}>
            {
                noToggle 
                ?
                null
                :
                <Radio.Group size={size || 'small'} buttonStyle='solid' className={style['radio-container']} value={timeType} onChange={e=>{
            ...
            <div style={{ display:'inline-flex'}}>
                ...
                }}><LeftOutlined /></div>
                {
                    timeType === '1' 
                    ?
                    <DatePicker ref={inputRef} size={ size || 'small'} locale={zhCN} allowClear={false} className={style['date-picker-container']} value={startDate} onChange={value=>{
                    ...
                    :
                    <RangePicker ref={inputRef} size={size || 'small'} locale={zhCN} allowClear={false} className={style['date-picker-container']} value={[startDate, endDate]} onChange={arr=>{
