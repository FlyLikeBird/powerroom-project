...
function AdjoinRateReport({ dispatch, costReport, global }){
    const { fieldList, currentField ,fieldAttrs, currentAttr, treeLoading, timeType, startDate, endDate, pagesize, containerWidth } = global;
    const { sourceData, currentPage, isLoading } = costReport;
    useEffect(()=>{
        dispatch({ type:'costReport/fetchAdjoinRate'});
        loadScript();
        return ()=>{
...
export default connect(({ costReport, global })=>({ costReport, global }))(AdjoinRateReport);