import { getExtremeReport, getCostReport, getEleReport, getWarningList, getSameRate, getAdjoinRate } from '../services/costReportService';
const initialState = {
    sourceData:[],
    isLoading:true,
    currentPage:1,
        ...
        *fetchEleReport(action, { call, put, select }){
            ...
            function* fetchEleReportCancelable(params){
                try {
                    ...
                    if ( Object.keys(currentAttr).length ){
                        let { data } = yield call(getEleReport, { company_id:companyId, attr_id:currentAttr.key, begin_date:startDate.format('YYYY-MM-DD'), end_date:endDate.format('YYYY-MM-DD'), time_type:timeType, energy_type: isRunning ? '2': '1' });
                        if ( data && data.code === '0'){
                            yield put({ type:'getCostReport', payload:{ data:data.data }});
                        }                      
                    } else {
